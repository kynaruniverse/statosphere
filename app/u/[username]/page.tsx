...
const VR = {
  ...
  border: '#C4D0B8', borderDeep: '#A8BCA0',
  text: '#161D14', muted: '#627056',
  accent: '#2D6A3F', accentMuted: '#D0E8D8', gold: '#6B8C3A', goldBg: '#E8F0D4',
...
export default function PublicProfilePage() {
  ...
  const handleRequestSeat = async () => {
    ...
    const { data: council } = await supabase
      .from('councils').select('id').eq('owner_id', profile.id).single()
    if (!council) { setReqError('Could not find their council.'); setRequesting(false); return }
    ...
    if (error) {
      setReqError(error.code === '23505' ? 'You already requested a seat.' : 'Failed to send request.')
    } else {
      setReqSent(true)
    }
    setRequesting(false)
  ...
  const formatDate = (d: string) => new Date(d).toLocaleDateString('en-GB', { month: 'long', year: 'numeric' })
  ...
  if (notFound) return (
    <main style={{ minHeight: '100svh', background: VR.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, fontFamily: VR.body, padding: '24px' }}>
    ...
    <main style={{ minHeight: '100svh', background: VR.bg, padding: '0 20px 80px', fontFamily: VR.body }}>
      <div style={{ maxWidth: 440, margin: '0 auto', paddingTop: 'calc(env(safe-area-inset-top,0px) + 24px)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          ...
          <Link href={isOwner ? '/dashboard' : '/'} style={{
            fontSize: 11, fontWeight: 700, fontFamily: VR.display, letterSpacing: '0.08em',
            padding: '5px 12px', border: `1px solid ${VR.border}`,
            borderRadius: 6, color: VR.muted, textDecoration: 'none', background: VR.card,
          }}>
        ...
        <div style={{
          ...
          boxShadow: '0 2px 16px rgba(22,29,20,0.07)',
        }}>
          {/* Name + XP */}
          <div style={{ padding: '20px 20px 0' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
              <div>
                ...
                <h1 style={{ fontSize: 22, fontWeight: 700, fontFamily: VR.display, color: VR.text, letterSpacing: '0.02em', lineHeight: 1.1, margin: 0 }}>
                  @{profile?.username}
                </h1>
                {profile?.full_name && (
                  <p style={{ fontSize: 13, color: VR.muted, fontStyle: 'italic', marginTop: 3 }}>{profile.full_name}</p>
                )}
                {profile?.location && (
                  <p style={{ fontSize: 11, color: VR.muted, marginTop: 4 }}>📍 {profile.location}</p>
                )}
              </div>
              <div style={{
                background: VR.surface, border: `1px solid ${VR.border}`,
            ...
            {/* Member since */}
            <p style={{ fontSize: 11, color: VR.muted, fontStyle: 'italic', marginBottom: 14 }}>
          ...
          {profile?.bio && (
            <div style={{ padding: '12px 20px', background: VR.surface, borderTop: `1px solid ${VR.border}` }}>
              <p style={{ fontSize: 13, lineHeight: 1.6, color: VR.text, fontStyle: 'italic', margin: 0 }}>
                {profile.bio}
              </p>
            </div>
          )}
          ...
          {profile?.becoming_statement && (
            <div style={{ padding: '12px 20px', borderTop: `1px solid ${VR.border}` }}>
              ...
              <p style={{ fontSize: 13, lineHeight: 1.55, color: VR.text, fontStyle: 'italic', margin: 0 }}>
                "{profile.becoming_statement}"
              </p>
            </div>
          )}
        ...
        <div style={{
          ...
          <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', fontFamily: VR.display, color: VR.muted, marginBottom: 18 }}>
            Build Stats
          </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              {stats.map((stat, i) => (
                <div key={stat.id}>
                  ...
                  <StatBar
                    icon={stat.stat_categories?.icon || ''}
                    name={stat.stat_categories?.name || ''}
        ...
        {/* Council seat request — only if not owner, logged in, requests open */}
        {!isOwner && viewerId && profile?.council_requests_open && (
          <div style={{
            ...
            <p style={{ fontSize: 13, color: VR.muted, fontStyle: 'italic', marginBottom: 14 }}>
              This person's council is open. Request a seat to help hold them accountable.
            </p>
                ...
                {showReqForm ? (
                  <div>
                    <textarea
                      ...
                      placeholder="Introduce yourself — why do you want a seat? (optional)"
                      rows={3}
                      style={{
                        ...
                        fontFamily: VR.body, marginBottom: 10, boxSizing: 'border-box',
                      }}
                      onFocus={e => { e.currentTarget.style.borderColor = VR.accent }}
                      onBlur={e => { e.currentTarget.style.borderColor = VR.border }}
                    />
                    {reqError && <p style={{ fontSize: 12, color: '#A0302A', marginBottom: 10 }}>{reqError}</p>}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <button onClick={() => setShowReqForm(false)} style={{ padding: '11px', background: VR.surface, color: VR.muted, border: `1px solid ${VR.border}`, borderRadius: 8, fontSize: 11, fontWeight: 700, fontFamily: VR.display, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: 'pointer' }}>
                        Cancel
                      </button>
                      <button onClick={handleRequestSeat} disabled={requesting} style={{ padding: '11px', background: VR.accent, color: '#FFFFFF', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 700, fontFamily: VR.display, letterSpacing: '0.08em', textTransform: 'uppercase', cursor: requesting ? 'not-allowed' : 'pointer', opacity: requesting ? 0.6 : 1 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: `0 2px 10px ${VR.accent}30`,
                    }}>
        ...
        {isOwner && (
          <Link href="/settings" style={{
            ...
            fontSize: 11, fontWeight: 700, fontFamily: VR.display,
            letterSpacing: '0.1em', textTransform: 'uppercase',
            color: VR.muted, textDecoration: 'none',
          }}>
            Edit your profile →
          </Link>
        )}
